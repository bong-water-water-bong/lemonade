...
void Server::auto_load_model_if_needed(const std::string& requested_model, const json& request_body) {
    ...
    // Forward per-request options (e.g. ctx_size) from the request body
    // to RecipeOptions so that auto-load respects the client's preferences.
    // When no request_body is provided (empty object), recipe defaults are used.
    router_->load_model(requested_model, info, RecipeOptions(info.recipe, request_body), true);
