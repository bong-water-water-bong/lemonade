...
static fs::path write_vllm_rocm_startup_shim() {
    fs::path dir = fs::temp_directory_path() / "lemonade-vllm-rocm-shim";
...
static void add_platform_env_for_backend(
    ...
    fs::path shim_dir = write_vllm_rocm_startup_shim();
...
void VLLMServer::load(const std::string& model_name,
    ...
    add_platform_env_for_backend(vllm_backend, env_vars);
